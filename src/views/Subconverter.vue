...
const configScriptBackend = (window.APP_CONFIG?.CONFIG_UPLOAD_BACKEND || process.env.VUE_APP_CONFIG_UPLOAD_BACKEND) + '/api.php'
...
const defaultBackend = window.APP_CONFIG?.SUBCONVERTER_BACKEND || process.env.VUE_APP_SUBCONVERTER_DEFAULT_BACKEND
const shortUrlBackend = (window.APP_CONFIG?.MYURLS_BACKEND || process.env.VUE_APP_MYURLS_DEFAULT_BACKEND) + '/short'
const configUploadBackend = (window.APP_CONFIG?.CONFIG_UPLOAD_BACKEND || process.env.VUE_APP_CONFIG_UPLOAD_BACKEND) + '/sub.php'
...
export default {
  data() {
    return {
      ...
      form: {
        sourceSubUrl: "",
        clientType: "",
        customBackend: this.getUrlParam() == "" ? (window.APP_CONFIG?.SUBCONVERTER_BACKEND || process.env.VUE_APP_SUBCONVERTER_DEFAULT_BACKEND || "https://api.v1.mk") : this.getUrlParam(),
  ...
  mounted() {
    ...
    if (window.APP_CONFIG) {
      // 获取构建时的默认值（从 process.env）
      const buildTimeBackend = process.env.VUE_APP_SUBCONVERTER_DEFAULT_BACKEND;
      const buildTimeShortUrl = process.env.VUE_APP_MYURLS_DEFAULT_BACKEND;
      
      // 添加订阅转换后端到列表
      if (window.APP_CONFIG.SUBCONVERTER_BACKEND && 
          window.APP_CONFIG.SUBCONVERTER_BACKEND !== buildTimeBackend) {
        const backendLabel = "自定义后端（运行时配置）";
        this.options.customBackend[backendLabel] = window.APP_CONFIG.SUBCONVERTER_BACKEND;
        // 设置为默认选中
        this.form.customBackend = window.APP_CONFIG.SUBCONVERTER_BACKEND;
      }
      
      // 添加短链接服务到列表
      if (window.APP_CONFIG.MYURLS_BACKEND && 
          window.APP_CONFIG.MYURLS_BACKEND !== buildTimeShortUrl) {
        const shortUrlValue = window.APP_CONFIG.MYURLS_BACKEND + '/short';
        const shortUrlLabel = "自定义短链（运行时配置）";
        // 检查是否已存在
        const exists = Object.values(this.options.shortTypes).includes(shortUrlValue);
        if (!exists) {
          this.options.shortTypes[shortUrlLabel] = shortUrlValue;
          // 设置为默认选中
          this.form.shortType = shortUrlValue;
        }
      }
    }
